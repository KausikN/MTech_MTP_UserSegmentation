...
class UserSegmentation_Cluster_Base:
    ...
    **params
        ...
        '''
        User Segmentation - Clustering - Algorithm
...
# Main Functions
# Array Functions
def Array_OneHot2Index_Naive(a, dtype={"numerical": float, "categorical": int}):
    '''
    Array - One Hot 2 Index (Naive - Without type info)
    '''
    # Check which columns are lists
    list_check = []
    category_cols = []
    for j in range(a.shape[1]):
        list_check.append(isinstance(a[0, j], list))
        if list_check[j]: category_cols.append(j)
    # Convert
    a_converted = []
...
def Array_Flatten_Naive(a, dtype=float):
    '''
    Array - Flatten (Naive - Without type info)
    '''
